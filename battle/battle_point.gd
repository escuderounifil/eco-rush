...
func configurar(
	novo_material: String,
	novo_nome_lixo: String,
	novo_numero: int,
	nova_cor: Color,
	nova_sprite: Texture2D
) -> void:
	...
	_criar_sprite(nova_sprite)
	_criar_visual()


func _criar_sprite(textura: Texture2D) -> void:
	if is_instance_valid(_sprite):
		return

	_sprite = Sprite2D.new()
	_sprite.name = "SpriteLixo"
	_sprite.texture = textura
	_sprite.position = Vector2.ZERO
	_sprite.scale = Vector2(0.046, 0.046)
	_sprite.z_index = 1
...
func _criar_visual() -> void:
	...
	_label_numero.add_theme_color_override(
		"font_outline_color",
		Color("202020")
	)
	...
	_label_interacao.add_theme_color_override(
		"font_outline_color",
		Color("202020")
	)
...
func _criar_barra_vida() -> void:
	...
	_barra_vida.add_theme_stylebox_override(
		"background",
		_style_barra(Color("252525"), Color("111111"))
	)

	_barra_vida.add_theme_stylebox_override(
		"fill",
		_style_barra(Color("c84545"), Color("7e2929"))
	)
...
func set_player_proximo(
	...
	if desbloqueado:
		...
		_label_interacao.text = "[B] LIXEIRA NECESSÁRIA"
		_label_interacao.add_theme_color_override(
			"font_color",
			Color("c9c9c9")
		)
